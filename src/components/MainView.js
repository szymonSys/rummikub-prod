...
const MainView = ({
  ...
  useEffect(() => {
    if (window.location.pathname === "/~17_syslo/rummikub/" && !checkKeys())
      resetGameType();
  });
  ...
  const createLinks = () => {
    if (window.sessionStorage.getItem("inGame") === "true")
      return (
        <>
          ...
          <Link to="/~17_syslo/rummikub/game">Game view</Link>
        </>
      );
    if (window.sessionStorage.getItem("inLobby") === "true")
      return (
        <>
          <h1>You aleady belong to game.</h1>
          <Link to="/~17_syslo/rummikub/waiting-for-players">Lobby</Link>
        </>
      );
    return false;
  };
