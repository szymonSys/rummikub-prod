...
const NewGame = ({
  ...
  const redirect = type => {
    if (!(founderName || slots)) return <Redirect to="/~17_syslo/rummikub/" />;
    switch (type) {
      case "singleplayer":
      case "localgame":
        return inGame ? <Redirect to="/~17_syslo/rummikub/game" /> : null;
      case "multiplayer":
        return inLobby ? (
          <Redirect to="/~17_syslo/rummikub/waiting-for-players" />
        ) : null;
      default:
        return <Redirect to="/~17_syslo/rummikub/error" />;
    }
  };
