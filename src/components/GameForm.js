...
const GameForm = ({ gameName, gamePassword, slotsChoice, handleChange }) => {
  ...
  const redirect = () => {
    if (window.sessionStorage.getItem("inGame") === "true")
      return <Redirect to={"/~17_syslo/rummikub/game"} />;
    if (window.sessionStorage.getItem("inLobby") === "true")
      return <Redirect to={"/~17_syslo/rummikub/waiting-for-players"} />;
    if (isCreated) return <Redirect to={"/~17_syslo/rummikub/new-game"} />;
    return null;
  };
